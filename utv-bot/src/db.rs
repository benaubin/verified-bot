...
impl DynamoDB {
    pub async fn new(table_name: &str) -> Self {
        ...
        Self { client, users_table_name: table_name.to_string(), guilds_table_name: "guilds".to_string() }
    }

    /// Gets claims data under the condition that DB access succeeds and it exists
    pub async fn get_user(&self, discord_id: u64) -> Option<UserData> {
        self.client
            .get_item()
    ...
    pub async fn get_role_config(&self, guild_id: GuildId) -> HashMap<String, u64> {
        self.client
            ...
            .key("guild_id", AttributeValue::N(guild_id.0.to_string()))
            .send()
            .await
            .ok()
            .map(|o| o.item().cloned())
            ...
            .map(|m| {
                ...
                for key in keys {
                    let temp: HashMap<String, u64> = match m.get(key) {
                        Some(AttributeValue::S(data)) => serde_json::from_str(data).unwrap_or(HashMap::new()),
                        _ => {
